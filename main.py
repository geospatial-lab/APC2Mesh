...
for subdir in subdirs:
    ...
    p2m_logger.info('description: Rebuttal - {}'.format(subdir))
    p2m_logger.info('Attention: {} @ feat_dim {}'.format(args.attention, 96))
    ...
    #     if pc_folder == 'gt': # if not pc_folder == 'fine':
    #         continue
    #     rebt_files[i] = pc_folder +'/'+pc_filepath.split('/')[-1]
    ...
    # for pc_filename in rebt_files:
        ...
        # pcc_as_gt = np.loadtxt(os.path.join(f'{args.data_path}/fine', pc_filename))  #TODO:save normals for pcc_out files in main_pcc.py
